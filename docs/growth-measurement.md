...
The public browser key is supplied as the GitHub Actions repository variable `PUBLIC_DEE_WEB_INGEST_KEY`. It is an ingestion-only browser value, not a dashboard/admin credential. Never use the unrestricted iOS production key. Missing configuration means no telemetry requests.
